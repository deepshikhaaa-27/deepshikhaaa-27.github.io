...
/* ── Auto-updating years+months of experience (career start: 3 Oct 2022) ── */
document.addEventListener('DOMContentLoaded', function(){
  ...
  var yLabel = years + (years === 1 ? ' year' : ' years');
  var mLabel = months + (months === 1 ? ' month' : ' months');
  var text = months > 0 ? (yLabel + ', ' + mLabel) : yLabel;
  el.textContent = text + ' of Oracle Cloud delivery';
});
...
(function(){
  ...
  var COLORS = ['#C74634','#C74634','rgba(199,70,52,0.5)','rgba(199,70,52,0.3)','rgba(34,39,43,0.35)','rgba(34,39,43,0.2)'];
    ...
    for(var i=0;i<pts.length;i++){
      for(var j=i+1;j<pts.length;j++){
        ...
        if(dist<MAX_DIST){
          ...
          ctx.strokeStyle = 'rgba(199,70,52,' + ((1-dist/MAX_DIST)*0.16) + ')';
