...
**Note:** the standalone `education.html` page is also gone — education now
lives only in the "Education" card on `about.html`. If your live GitHub repo
still has an old `education.html` file in it, delete it there (Code → click
the file → trash-can icon → commit) so the nav doesn't have a dead link.

## Files to upload — ALL of these, together

- `index.html` (Home)
- `about.html` (redesigned: profile card, proficiency bars, SME goals, mini timeline, Education card)
...
- `Add institution & years` (x2, in the About page's Education card)
